...
<li><span>徐石头</span> 👍（2） 💬（1）<p>1. 操作系统调用栈，浏览器前进后退功能
...
用Go实现的数组栈和链表栈  https:&#47;&#47;github.com&#47;xushuhui&#47;algorithm-and-data-structure&#47;tree&#47;master&#47;datastructure&#47;stack</p>2023-03-02</li><br/><li><span>Yj.yolo</span> 👍（1） 💬（1）<p>1、操作系统中的调用栈：在操作系统中，每个进程都有一个独立的调用栈，用于存储函数调用的返回地址和参数。当程序执行一个函数时，它的参数和返回地址被压入该进程的调用栈中。当函数返回时，这些参数和返回地址被弹出栈。
...
6、撤销操作：在编辑器中，撤销操作通常也可以使用栈来实现，将每次编辑的操作记录在栈中，撤销时将最近的操作出栈并撤销。</p>2023-03-17</li><br/><li><span>Fang</span> 👍（0） 💬（0）<p>学完就忘， </p>2024-07-30</li><br/>
</ul>

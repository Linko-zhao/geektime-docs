...
<li><span>neohope</span> 👍（9） 💬（1）<p>一、数据结构
...
系统的idel进程只有一个，如果多个cpu同时空闲，会不会有问题啊？空闲进程不用per_cpu吗？</p>2021-07-07</li><br/><li><span>pedro</span> 👍（17） 💬（2）<p>并不会，进程进入等待只是进程状态发生了改变，进程还未让出当前CPU的执行权，待调度后，即 krlschedul()，会寻找已经准备好的其它进程，切换CPU上下文，让出CPU，此时该进程才会真正的停止。

所以调度函数至关重要！</p>2021-07-07</li><br/><li><span>geek2020</span> 👍（3） 💬（3）<p>好奇很多资料说java执行了Thread.sleep()后，会主动让出CPU的使用权，是怎么做到的？</p>2021-07-15</li><br/><li><span>Feen</span> 👍（2） 💬（1）<p> 进程进入等待状态后，进程本身处于等待状态，进程被剥夺CPU使用权是通过krlschedul()函数剥夺的，而进程本身并没有调用krlschedul()函数，所以不会立即停止运行，而剥夺CPU使用权是由krlschedul()函数控制的，当保存当前进程寄存器和栈，并且准备好下一个进程的运行函数后，才会让当前进程退出CPU而停止运行了。</p>2021-07-13</li><br/><li><span>青玉白露</span> 👍（1） 💬（1）<p>进程不会马上停止运行，调用krlschedul()之后，会调度新的进程。</p>2021-07-07</li><br/><li><span>Ivan.Qi</span> 👍（0） 💬（3）<p>遇到一个问题，暂时没什么头绪
...
4. 最后提示
   当前进程: kernelthread-a,犯了不该犯的错误:13, 所以要杀</p>2022-08-24</li><br/><li><span>我是新手ABC</span> 👍（0） 💬（2）<p>有个疑问：如果空转函数一直运行的话，也就是CPU不是在运行用户代码就是在运行空转函数，那CPU的使用率会不会一直是100%呢？</p>2022-05-23</li><br/><li><span>艾恩凝</span> 👍（0） 💬（3）<p>自己建的工程，怎么新建空转进程的时候 运行到krlthread_kernstack_init 函数中，就会报14号异常，这是怎么回事，就是初始化内核栈的时候，按道理说这都是物理地址，怎么还是会进入14 异常</p>2022-05-05</li><br/><li><span>胡涂涂</span> 👍（0） 💬（1）<p>这个调度机制很像嵌入式中实时操作系统的多任务调度，空转进程对标空闲任务。空转任务中，可以做一些维护性的任务，不能让cpu闲下来，哈哈哈</p>2022-04-26</li><br/><li><span>阿顺</span> 👍（0） 💬（1）<p>想咨询下进程如何决策何时要等待，并让出cpu？</p>2021-08-28</li><br/><li><span>blentle</span> 👍（0） 💬（1）<p>好奇，这个空转进程是我们平时所说的0号进程吗，1号进程的创建是fork它吗</p>2021-07-07</li><br/><li><span>罗 乾 林</span> 👍（0） 💬（1）<p>进程进入等待状态后，这进程不会立马停止运行，要等到调用krlschedul函数后</p>2021-07-07</li><br/>

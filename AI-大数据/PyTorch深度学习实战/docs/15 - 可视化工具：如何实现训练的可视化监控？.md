...
for n_iter in range(10): # 随机获取loss值
loss = 0.2 * np.random.randn() + 1 # 随机产生accuracy
accuracy = 0.1 * np.random.randn() # 更新窗口图像,同时更新loss和accuracy
...
for n_iter in range(100): # 随机获取loss值
loss = 0.2 * np.random.randn() + 1
accuracy = 0.2 * np.random.randn() + 1 # 更新窗口图像

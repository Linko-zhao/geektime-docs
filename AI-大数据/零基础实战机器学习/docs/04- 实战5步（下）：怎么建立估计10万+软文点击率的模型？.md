...
normalize = item[1] # 训练模型
linereg_model = LinearRegression(fit_intercept = fit_intercept, normalize = normalize)
linereg_model.fit(xtrain, ytrain) # 预测测试集对的y值
y_pred = linereg_model.predict(xtest) # 给出模型评分(使用验证集)
...
TypeError: LinearRegression.**init**() got an unexpected keyword argument &#39;normalize&#39;</p>2023-09-22</li><br/><li><span>Untitled</span> 👍（0） 💬（1）<p>拟合出来的模型要怎么保存起来，下次有新的数据集能快速预测？</p>2023-09-12</li><br/>
</ul>

...
![](https://static001.geekbang.org/resource/image/f3/dd/f3be95c49af5bed1965654dd79db7bdd.png?wh=1584%2A196)  
那我们再看一下Swap空间，使用了1.5GB (used 1542144KB)。输出的结果如下图，简单计算一下，1.5GB + 512MB，结果正好是mem\_alloc这个程序申请的2GB内存。

![](https://static001.geekbang.org/resource/image/e9/3f/e922df98666ab06e80f816d81e11883f.png?wh=1658%2A198)
...
![](https://static001.geekbang.org/resource/image/35/50/35b806bd26e089506d909d31f1e87550.png?wh=1496%2A340)

接下来，调用mem\_alloc之后，Swap空间就被使用了。

![](https://static001.geekbang.org/resource/image/e2/ec/e245d137131b1yyc1e169a24fd10b5ec.png?wh=1618%2A200)
...
![](https://static001.geekbang.org/resource/image/21/1d/212f4dde0982f656610cd8a3b293051d.png?wh=630%2A240)  
...
![](https://static001.geekbang.org/resource/image/46/a7/46a1a06abfa2f817570c8cyy5faa62a7.png?wh=1920%2A495)
...
![](https://static001.geekbang.org/resource/image/6a/11/6aa89d2b88493ddeb7d37ab9db275811.jpeg?wh=3200%2A1800)
...
<li><span>伟平</span> 👍（20） 💬（2）<p>k8s下容器貌似不能用swap</p>2020-12-07</li><br/><li><span>莫名</span> 👍（10） 💬（1）<p>Memory Cgroup 参数 memory.swappiness 起到局部控制的作用，前提是节点开启了 swap 功能，不然无论如何设置都无济于事。</p>2020-12-07</li><br/><li><span>Khirye</span> 👍（8） 💬（2）<p> 想问下老师，对于最近k8s宣布放弃docker有什么看法？</p>2020-12-11</li><br/><li><span>kimoti</span> 👍（8） 💬（2）<p>既然memory.swappiness设置为0了,Swap分区是不会有数据写入的。</p>2020-12-07</li><br/><li><span>po</span> 👍（2） 💬（5）<p>老师，文章中你说的这两句话好像是矛盾的，swappiness设置为0的时候，到底会不会回收匿名内存呢？
...
2. 值为 0 的时候，当系统中空闲内存低于一个临界值的时候，仍然会释放匿名内存并把页面写入 Swap 空间。</p>2020-12-15</li><br/><li><span>haha</span> 👍（2） 💬（1）<p>所以对于开启了swap，且swap空间够大的前提下，goup的mem limit没用咯?</p>2020-12-15</li><br/><li><span>Geek4329</span> 👍（2） 💬（1）<p>感谢老师的分享，学习的越多，越感到自己的自信😼

学完这个课程，并完全吸收的话，是不是可以说自己接近精通容器技术了😁</p>2020-12-07</li><br/><li><span>chong chong</span> 👍（1） 💬（1）<p>老师，k8s pod默认的memory.swappiness 值是60，如何设置才能使得默认是0呢？</p>2021-05-11</li><br/><li><span>谢哈哈</span> 👍（1） 💬（1）<p>已经设置memory.swappiness参数，全局参数swappiness参数失效，那么容器里就不能使用swap了</p>2020-12-07</li><br/><li><span>刘宏</span> 👍（1） 💬（1）<p>请问一下在容器里如何加快page cache的释放速度，原因是发现scp的时候，page cache会很快将内存占满，速度会骤然下降，drop cache以后，速度能显著提升；配置了vm.vfs_cache_pressure为1w，依旧在容器里没改善</p>2021-10-03</li><br/><li><span>BertGeek</span> 👍（1） 💬（0）<p>1、swappiness（&#47;proc&#47;sys&#47;vm&#47;swappiness） 的取值范围在 0 到 100 之间，我们可以记住下面三个值：

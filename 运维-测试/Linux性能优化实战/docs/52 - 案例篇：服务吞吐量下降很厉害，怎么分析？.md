...
<li><span>2xshu</span> 👍（17） 💬（2）<p>老师，有个疑问。
套接字优化部分，你用ss -s输出的两个队列，根据“关于 Linux 网络，你必须知道这些（下）”你讲的内容，当链接处于listening状态是，Send-Q和Recv-Q都是半链接队列，但是你这里却都是调的全连接队列啊？不是应该调整tcp_max_syn_backlog吗？</p>2019-03-25</li><br/><li><span>ninuxer</span> 👍（6） 💬（3）<p>打卡day55
缺乏由现象联想到可能原因的系统性思维～</p>2019-03-25</li><br/><li><span>xfan</span> 👍（4） 💬（2）<p>内核选项 tcp_tw_reuse，不是直接修改内核参数就好了么，为什么还有修改后的tag:3 ,这里不太清楚</p>2019-03-28</li><br/><li><span>泡泡</span> 👍（4） 💬（1）<p>wrk命令-c参数用来模拟连接数为1000，
为什么输出中的连接数有1910，不理解</p>2019-03-26</li><br/><li><span>Maxwell</span> 👍（4） 💬（1）<p>在公司局域网下做性能测试，如何判断网络会不会成为压测的瓶颈呢？也就是说如果开了500线程进行压测，会不会因为网络瓶颈，导致请求无法发送到服务器端？</p>2019-03-25</li><br/><li><span>burner</span> 👍（1） 💬（1）<p>老师，系统cpu只用了一半，但是就出现502和499的请求错误，是否意味这应用服务已经过载，还是系统连接数过载，查看netstat发现有28万失败的连接尝试，</p>2019-08-13</li><br/><li><span>腾达</span> 👍（1） 💬（2）<p>net.ipv4.tcp_tw_reuse = 1 这里是影响到socket的客户端（nginx作为一个客户端连接php的服务端）的行为吗？ 不是影响到服务端的time_wait数量？ 我弄了个tomcat，用ab压测，tw_use=1, 用ss -s看time_wait 还很高啊，1万多。</p>2019-04-15</li><br/><li><span>子杨</span> 👍（0） 💬（2）<p>服务端端口号不是近似无限的吗，这里怎么又有限制了。</p>2020-07-22</li><br/><li><span>腾达</span> 👍（0） 💬（1）<p>老师，针对我提的问题，您的回复是：“不过你可以docker exec到容器内部查看”，我已经逐一对比过容器内的、我已知的参数了。未发现不同。您能否把最后一次的配置参数上传一下到github？</p>2019-04-08</li><br/><li><span>腾达</span> 👍（0） 💬（1）<p>有2个问题：
...
发现都是一样的。不知道哪里有问题。
老师，你能把优化最后的配置文件上传一份到github吗？</p>2019-04-05</li><br/><li><span>腾达</span> 👍（0） 💬（1）<p>有3个问题：
...
3.从docker运行后续集个tag=2，3的镜像开始，例如：docker run --name nginx --network host --privileged -itd feisky&#47;nginx-tp:3 这个命令拉的镜像的参数修改了哪些地方？从哪里可以看到这些参数？</p>2019-04-01</li><br/><li><span>code2</span> 👍（0） 💬（1）<p>用桌面linux分析服务器性能，有些勉强。</p>2019-03-28</li><br/><li><span>Maxwell</span> 👍（0） 💬（1）<p>Sar测试的只是网络的发送和接收数据吧，好像并不能发现网络的瓶颈导致压测请求无法发送至服务器端？</p>2019-03-26</li><br/><li><span>Maxwell</span> 👍（0） 💬（1）<p>这个火焰图咋分析？我这边和你的好像不太一样，系统我也是ubantu18.04
</p>2019-03-26</li><br/><li><span>Maxwell</span> 👍（0） 💬（1）<p>运行最后一次docker镜像，wrk测试结果中还是有很多error(read ),请问这个error指的是什么错误呢？

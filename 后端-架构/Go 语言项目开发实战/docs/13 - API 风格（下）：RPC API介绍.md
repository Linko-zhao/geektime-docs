...
<li><span>pedro</span> 👍（70） 💬（1）<p>假定希望用RPC作为内部API的通讯，同时也想对外提供RESTful API，又不想写两套，可以使用gRPC Gateway 插件，在生成RPC的同时也生成RESTful web  server。</p>2021-06-24</li><br/><li><span>Geek_e7af5e</span> 👍（43） 💬（4）<p>这里提供一些更新说明（刚踩过的坑）

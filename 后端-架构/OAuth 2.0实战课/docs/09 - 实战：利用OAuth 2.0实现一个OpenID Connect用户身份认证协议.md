...
3. 如果 access_token 过期（即登录已失效），客户端仍然可以用 id_token 解析出用户信息，这岂不是更不合理？</p>2020-07-18</li><br/><li><span>工资不交税</span> 👍（12） 💬（8）<p>在应用oss中，一端退出是不是还需要通知认证服务？不然认证服务的状态还是登录，那其他端还是能直接登录，甚至自己都没法退出。

</p>2020-07-21</li><br/><li><span>DB聪</span> 👍（9） 💬（10）<p>图3中”重复上述1-6”陈述单点登录的描述感觉有点难理解，原因在第3步，如果分别登陆a1.com、a2.com、a3.com的时候，都有第3步的参与，那是否意味着End User每次都需要输入用户名和密码呢？</p>2020-07-18</li><br/><li><span>成立-Charlie</span> 👍（7） 💬（4）<p>老师，您好！关于ID Token和Access Token，还需要再请教一下。
...
另外，当我们继承一个认证服务的时候，ID Token是我们评断认证服务是否实现OIDC的标准吗？</p>2020-08-06</li><br/><li><span>leros</span> 👍（6） 💬（1）<p>能不能比较下基于SAML和基于OIDC的SSO？一些大的授权服务平台可能二者都提供，不太清楚具体实践中如何选择</p>2020-07-19</li><br/><li><span>stubborn</span> 👍（5） 💬（2）<p>老师您好，两个问题请教下。
1. access_token失效了可以用refresh_token重新获取。id_token失效了怎么办，这块有没有规范？ keycloak的实现中使用refresh_token可以重新生成id_token。 
2. Oauth2其实也可以实现认证的功能，只要把access_token定义包含认证信息就可以了，这样使用access-token就类似id_token了。不太明白为何OIDC需要突出这部分的定义？</p>2020-08-16</li><br/><li><span>哈德韦</span> 👍（4） 💬（4）<p>传统的登录基于Session，是不是使用JWT Token方案，就不需要Session了（也不需要Cookie参与了）？</p>2020-07-19</li><br/><li><span>往事随风，顺其自然</span> 👍（4） 💬（5）<p>代码中access_token中就包含用户信息，获取accesstoken 时候需要带上id_token中的用户唯一标识？</p>2020-07-18</li><br/><li><span>哈德韦</span> 👍（2） 💬（1）<p>前面的课程里讲到，一般来说 JWT 有个缺陷，为了克服“覆水难收”，需要一个额外的用户粒度的密钥管理。那么，这个用户粒度的密钥管理是针对 access_token 的吗？id_token 的密钥，也需要到用户粒度吗？</p>2020-07-19</li><br/><li><span>在路上</span> 👍（1） 💬（1）<p>王老师，单点登录的步骤中，a2.com需要去解析，id_token的 ID值么？</p>2020-07-21</li><br/><li><span>冷锋</span> 👍（1） 💬（2）<p>CAS和SSO有什么区别？</p>2020-07-20</li><br/><li><span>许灵</span> 👍（1） 💬（4）<p>好像现在的第三方登录都是通过access_token来获取用户信息的，这是不是表示access_token与id_token合并了？</p>2020-07-20</li><br/><li><span>奕</span> 👍（1） 💬（5）<p>单点登陆的那个流程图没有看懂， 为什么 a1.com 输完用户名密码授权登陆后， 在访问 a2.com, a3.com 还有走 1-6的步骤呢？ 这就不符合一次输入授权 多处登陆了啊</p>2020-07-18</li><br/><li><span>往事随风，顺其自然</span> 👍（1） 💬（2）<p>userinfo 端点是啥意思，就是请求时候，access _token会带上用户唯一标识？app_id算不算唯一标识，和用户绑定关系</p>2020-07-18</li><br/><li><span>永旭</span> 👍（0） 💬（1）<p>提个建议.  用例代码就算不用spring , 也得用maven啊, 这种纯web还得陪容器,只是增加了复杂而已. </p>2020-11-11</li><br/>
</ul>

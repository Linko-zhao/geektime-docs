...
}</p>2022-01-15</li><br/><li><span>Geek_62c21a</span> 👍（6） 💬（1）<p>老师，有两个问题请教您
...
ready = true
groupSignal.L.Unlock()
groupSignal.Broadcast()</p>2022-04-25</li><br/><li><span>aoe</span> 👍（6） 💬（1）<p>读到这里，明白了 Go 的并发不是万能的！
...
3. 有锁的地方就有江湖，高并发下的性能主要拼的是算法，没有一门语言有压倒性优势</p>2022-01-18</li><br/><li><span>Calvin</span> 👍（5） 💬（9）<p>老师，Go 中没有可重入的锁吗？</p>2022-01-15</li><br/><li><span>非梧桐不止</span> 👍（4） 💬（2）<p>老师，既然共享内存的性能比channel好，为什么Go倡导用通信去共享内存呢？</p>2022-05-27</li><br/><li><span>singularity of space time</span> 👍（3） 💬（2）<p>老师，请问一下Go在没有volatile的情况下如何保证共享变量在不同Goroutine的可见性？如果可见性不能保证的话，那么CAS的正确性应该也不能保证吧？还是说CAS内部的实现已经保证了可见性呢？</p>2022-02-06</li><br/><li><span>文经</span> 👍（3） 💬（1）<p>白老师，我觉得条件变量的例子完全可以用channel来实现，代码逻辑还会更简单一些。
   使用条件变量，跟Mutex一样，是基于性能的考虑吗？</p>2022-01-21</li><br/><li><span>文经</span> 👍（3） 💬（1）<p>白老师：
   “atomic 原子操作可用来同步的范围有比较大限制，只能同步一个整型变量或自定义类型变量。如果我们要对一个复杂的临界区数据进行同步，那么首选的依旧是 sync 包中的原语。”
   这里说的整型变量和自定义类型变量，是不是可以理解为一个字长的大小。在64位cpu上就是8个字节。因为CPU通过数据总线，一次从内存中最多只能获取一个字长的信息。所以atomic的限制也是一个字长。</p>2022-01-21</li><br/><li><span>Calvin</span> 👍（3） 💬（5）<p>老师，sync 包中还有其他的一些同步原语，后面会继续说下吗？比如前面课程和本节课中都有使用到的 sync.WaitGroup 就没有说到，直接就使用上了，另外还有 sync.Map，sync.Pool 等。</p>2022-01-15</li><br/><li><span>吃橘子的汤圆🐳</span> 👍（2） 💬（1）<p>请问使用Csp比使用共享内存的好处体现在哪里</p>2022-12-13</li><br/><li><span>冯仁彬</span> 👍（2） 💬（1）<p>以下代码会产生死锁:
   func main() {
   var mu1 sync.Mutex
   var mu2 sync.Mutex
   mu1.Lock()
...
4. 文中说：“甚至，如果拷贝的时机不对，比如在一个 mutex 处于 locked 的状态时对它进行了拷贝，就会对副本进行加锁操作，将导致加锁的 Goroutine 永远阻塞下去。” 这里不是互不相关吗？ 为什么还会对副本加锁呢？

5. 另外，好像也没讲到空结构体在channel中传递的好处？

6. $GOROOT&#47;src&#47;runtime&#47;internal&#47;asm_amd64.s 这个文件未找到（Mac Book 上面）。</p>2022-01-20</li><br/>

...
![](https://static001.geekbang.org/resource/image/2f/f5/2f59ace16127a243ebc620d9b4d78cf5.jpg?wh=2900x1857 "图3 读 Key 数据")
...
read 存储了部分写入 Map 的内容，用来加速读操作。而 dirty 存储了全量内容，需要加锁才能读写数据。 </p>2024-12-30</li><br/><li><span>lJ</span> 👍（0） 💬（1）<p>1. 只支持 []byte 类型的数据存储，不支持复杂的数据结构，需要自行序列化和反序列化数据，增加了开发复杂度。2. 使用环形缓冲区存储数据，数据写入时是连续分配的，但删除时只标记为无效，不回收空间。导致内存利用率降低。</p>2024-12-25</li><br/><li><span>快叫我小白</span> 👍（0） 💬（1）<p>runtime.GC 这个函数为何在函数内外都调用一次呀？而且测试函数似乎没产生需要回收的临时结构体，我们调用GC函数仅仅是为了观察垃圾回收的扫描时间吗？</p>2024-12-25</li><br/>
</ul>

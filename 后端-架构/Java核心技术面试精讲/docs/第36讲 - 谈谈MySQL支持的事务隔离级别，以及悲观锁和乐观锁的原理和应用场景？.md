...
<li><span>lizishushu</span> 👍（113） 💬（2）<p>mybatis架构自下而上分为基础支撑层、数据处理层、API接口层这三层。

基础支撑层，主要是用来做连接管理、事务管理、配置加载、缓存管理等最基础组件，为上层提供最基础的支撑。
数据处理层，主要是用来做参数映射、sql解析、sql执行、结果映射等处理，可以理解为请求到达，完成一次数据库操作的流程。
API接口层，主要对外提供API，提供诸如数据的增删改查、获取配置等接口。</p>2018-07-31</li><br/><li><span>一首歌一种心情一段路</span> 👍（53） 💬（9）<p>悲观锁和乐观锁在哪儿用的.......平时写sql没接触过啊</p>2018-11-24</li><br/><li><span>文彦</span> 👍（14） 💬（1）<p>晓峰老师。最近感觉基础有点差，开始看jdk的源码，主要是结合api来看。但是感觉事倍功半，有什么好的建议吗？</p>2018-11-08</li><br/><li><span>clz1341521</span> 👍（13） 💬（2）<p>mysql inndb默认可重复读级别，不会出现幻读。
mybatis架构自下而上分为基础支撑层、数据处理层、API接口层这三层。

基础支撑层，主要是用来做连接管理、事务管理、配置加载、缓存管理等最基础组件，为上层提供最基础的支撑。
数据处理层，主要是用来做参数映射、sql解析、sql执行、结果映射等处理，可以理解为请求到达，完成一次数据库操作的流程。
API接口层，主要对外提供API，提供诸如数据的增删改查、获取配置等接口。</p>2018-08-11</li><br/><li><span>L.B.Q.Y</span> 👍（12） 💬（2）<p>mysql（innodb）的可重复读隔离级别下，为什么可以认为不会出现幻像读呢?</p>2018-07-31</li><br/><li><span>饭粒</span> 👍（8） 💬（5）<p>SQL标准的隔离级别可重复读还是有幻读问题的。 InnoDB 和 XtraDB存储引擎 通过多版本并发控制（ MVCC， Multiversion Concurrency Control） 解决了幻读的问题。</p>2019-02-12</li><br/><li><span>李二木</span> 👍（5） 💬（2）<p>说到mybatis,就想起了分页，现在绝大多分页都用到了pagehelper插件，我想问下老师为啥mybatis没有设计一个好用的分页了？</p>2018-07-31</li><br/><li><span>kitten</span> 👍（1） 💬（1）<p>mysql，可重复读的隔离级别，也在有gap间隙锁吧？</p>2018-08-01</li><br/><li><span>互联网联络员</span> 👍（0） 💬（1）<p>老师是不是写错了，在可重复读隔离级别中，基于锁的并发控制 DBMS实现保持读取和写入锁定（在选定数据上获取），直到事务结束。但是，range-locks没有被管理 ，因此可能会发生幻像读取。

事物的隔离级别增强分别解决由脏读、不可重复读再到幻读，只有串行化能解决三者，但是损失了效率。</p>2018-10-22</li><br/><li><span>任鹏斌</span> 👍（13） 💬（6）<p>对于mysql四个隔离级别中的不可重复读的最后一句不是很理解。不可重复读应该是不能避免幻读，为什么说是不产生幻读呢？</p>2018-08-12</li><br/><li><span>郝国梁</span> 👍（6） 💬（0）<p>乐观锁 悲观锁 脏读 幻读 不可重复读 CAS MVCC 隔离级别 锁队列 2PC TCC</p>2018-07-31</li><br/><li><span>anji</span> 👍（4） 💬（0）<p>0.sql工厂-主要设定数据库连接信息
1.接口层-主要有mapper接口用于对外提供具体的sql执行方法
2.xml文件-有具体的sql实现语句，以及数据库字段对应java类字段的映射关系，每个mapper对应每个数据库表
</p>2018-07-31</li><br/><li><span>Geek_j5i7lb</span> 👍（3） 💬（1）<p>杨老师 您好，关于“可以简单认为MySQL在可重复读级别不会出现幻象读”没能理解，个人认为正是因为可重复读，即每个事务保存了快照，才导致了幻读的现象。想要解决幻读，只能加共享锁或者排它锁吧。 可能是因为知识浅导致无法理解，还望方便的话，您能点拨一下，感谢</p>2018-08-22</li><br/><li><span>Allen</span> 👍（3） 💬（0）<p>请问什么时候可以细说一哈ORM映射😋</p>2018-08-01</li><br/><li><span>sunnyrqw</span> 👍（1） 💬（0）<p>评论有说悲观锁和乐观锁在哪里用，说下我的理解
悲观锁和乐观锁是概念，如专栏说的
悲观锁是完全排他性的，我在执行的时候，你不能动，具体实现 java的synchronized、ReentrantLock
乐观锁就是我在执行的时候不限制你的操作，通过对比前后的值看有没人操作，没人操作就更新，有人操作就重来一次，具体实现是java cas</p>2020-10-13</li><br/>
</ul>

...
![](https://static001.geekbang.org/resource/image/ca/a7/cab2cf6bc91f261b273d614336086fa7.jpg?wh=2700%2A2674)
...
<li><span>Jxin</span> 👍（2） 💬（2）<p>重分布这个有疑惑: 如果我有多个关联查询呢?每次关联查询都重分布？这样重分布就可能是个死循环了。因为A关联查询和B关联查询的重分布,可能会相互影响。</p>2020-09-23</li><br/><li><span>有铭</span> 👍（0） 💬（1）<p>Grace这个最早使用GHJ算法数据库没有查到啊？

很久以前就困惑传统数据库在分表分库后如何解决joint的问题。今天看到这篇文章后豁然开朗</p>2020-09-23</li><br/><li><span>Jowin</span> 👍（2） 💬（0）<p>分布式数据库的并行查询，底层依赖的是和大数据计算平台相同的并行计算技术。可以想见，在Spark上支持SQL查询，其实是一样的原理。这一讲非常棒，把分布式数据库和大数据技术串起来了！</p>2021-03-08</li><br/><li><span>Geek_64affe</span> 👍（0） 💬（0）<p>思考题最主要的问题我觉得应该是如何确定 大&#47;小 表</p>2022-05-10</li><br/><li><span>wy</span> 👍（0） 💬（2）<p>老师有个问题不理解 假设两个表数据量一样大都是n,那么嵌套循环的复杂度是n*n ,而排序归并的复杂度应该是nlogn+nlogn+2*n约等于nlogn。这样看的话排序归并的效率更高一点，但是文中你说成本更高一些，体现在什么地方呢</p>2021-01-26</li><br/><li><span>black_mirror</span> 👍（0） 💬（0）<p>HI，Ivan老师好
1、GHJ算法，是不是每次只把inner表bucket加入内存，而outer表的bucket一直在磁盘中，进行2者的比较？
2、observer node4 节点第2个工作线程是不是应该叫4-2 ？</p>2020-12-28</li><br/><li><span>幼儿编程教学</span> 👍（0） 💬（0）<p>大表join，查询过来的时候，再做重分布？在节点间移动数据？这样不会很慢？是不是我的理解哪里有问题？这种指的是olap吧？</p>2020-11-22</li><br/><li><span>星之柱</span> 👍（0） 💬（0）<p>h如果选择的inner表ash不均衡的时候，就退化成了嵌套循环</p>2020-11-18</li><br/><li><span>游弋云端</span> 👍（0） 💬（0）<p>“在计算逻辑允许的情况下，建立阶段选择数据量较小的表作为 Inner 表”，我的问题就是在什么情况下，系统无法根据数据量决定 Inner 表呢？
答复：如果连接属性本身内容重复较多，但是表格很大，这样反而选择较大的表格作为 Inner 表，可以根据内容相同，从而节省Hash的计算资源。想到这种场景。</p>2020-09-24</li><br/><li><span>赵见跃</span> 👍（0） 💬（0）<p>哈希算法，“在计算逻辑允许的情况下，建立阶段选择数据量较小的表作为 Inner 表”，我的问题就是在什么情况下，系统无法根据数据量决定 Inner 表呢？这个问题，也很困惑，大家给指点一下呀，谢谢。</p>2020-09-24</li><br/><li><span>piboye</span> 👍（0） 💬（0）<p>复制表，写的时候也是异步复制到其他节点上？</p>2020-09-23</li><br/><li><span>licl1008</span> 👍（0） 💬（0）<p>数据量小的一侧不是构建侧，这种情况不能单单根据数据量来决定innner</p>2020-09-23</li><br/><li><span>扩散性百万咸面包</span> 👍（0） 💬（0）<p>我对重分布的理解：按照某个非分区键再做一遍重分区。

对于这个思考题，应该是两个关联表的大小都远远超过了单机的内存？所以必须要上分布式？</p>2020-09-23</li><br/>
</ul>

...
3. 为什么在 addOrg 加事务？里面就一个 save 操作，是否没必要加@Transactional
4. 很多用@Autowired 注入的属性，有些是 final，有些不是 final，这个有什么讲究吗？</p>2023-01-17</li><br/><li><span>Breakthrough</span> 👍（1） 💬（1）<p>钟老师，DDD设计完成后的交付文档一般长啥样，是不是也要做类似设计需求说明书的WORD档。如果是，是否有相关的模板参考下？</p>2023-07-17</li><br/><li><span>龙腾</span> 👍（1） 💬（1）<p>老师请教一下，“第三，应用服务可以读写数据库。”这块没太明白，什么情况下读写数据库不包含领域知识呢？咱们的数据库设计不就是按照领域来划分设计的吗，只要读写都是在领域内进行的吧？</p>2023-01-02</li><br/><li><span>开心</span> 👍（0） 💬（1）<p>看到这里，没有感受到太多面向对象，更多是面向过程的实现。领域业务大都是放在了没有属性只有方法的领域服务，更多的是如何定义领域服务模块。有点不解。</p>2024-04-25</li><br/><li><span>路上</span> 👍（0） 💬（1）<p>类似撤销这部分校验逻辑，目前看领域对象自己就可以校验，目前是在外层进行的校验之后调用领域方法进行状态变更
...
5. 一般直接用私有，避免被外部访问，即使是包内，封装性一步到位；
6. 审计字段一般不会被业务使用，可以没有getter，其他字段看情况。</p>2023-04-07</li><br/><li><span>Geek_e10c1b</span> 👍（0） 💬（1）<p> 老师，领域对象中不访问数据库，那如何对某些属性进行懒加载呢</p>2023-03-28</li><br/>

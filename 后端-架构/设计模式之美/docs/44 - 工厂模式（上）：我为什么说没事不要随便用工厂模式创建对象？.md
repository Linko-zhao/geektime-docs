...
验证了一下，发现 *static*方法可以被重写，_static_ 与 _abstract_ 是冲突的, 不能同时修饰一个方法；而且，如果用子类重写了父类的static方法，这时候让父类的引用指向子类对象，然后调用该*static*方法，这时调用的是父类的*static*方法，也就是不支持“多态”，这也解释了为什么*static* 与 *abstract*冲突。

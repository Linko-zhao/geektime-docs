...
以上就是我的一些疑问，还麻烦唐老师指导一下，Rust 的一些概念有点让人头晕。</p>2023-11-02</li><br/><li><span>一个人旅行</span> 👍（5） 💬（1）<p>1. 默认情况下，struct不能进行比较，需要为Shape类型实现PartialEq trait特征。2. 方式一：使用枚举，该枚举类型的枚举值表示一个类型。

...
首先，显然 `ranges` 里的 `begin` 和 `end` 返回的迭代器（_b_ 和 _e_）能够表达一个范围，[_b_, _e_) 应当组成一个半闭半开区间。
...
- 取用视图（`views::take`）：由一个视图的前 _n_ 个元素组成的视图。
- 条件取用视图（`views::take_while`）：只要条件满足，即一直返回视图中的元素；条件不满足即终止。
- 丢弃视图（`views::drop`）：跟 `take` 相反，丢弃一个视图的前 _n_ 个元素后剩下的元素组成的视图。

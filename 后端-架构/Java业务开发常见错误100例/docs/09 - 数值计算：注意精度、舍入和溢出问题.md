...
7.ROUND_HALF_EVEN：奇偶版四舍五入取整，如果舍弃部分左边的数字为奇数，则作 ROUND_HALF_UP；如果它为偶数，则作ROUND_HALF_DOWN，会根据舍弃部分的奇偶性来选择进位的是四舍五入还是五舍六入。8. ROUND_UNNECESSARY：要求传入的数必须是精确的，如 1 和 1.0 都是精确的，如果为 1.2 或者 1.6 之类的均会报 ArithmeticException 异常。

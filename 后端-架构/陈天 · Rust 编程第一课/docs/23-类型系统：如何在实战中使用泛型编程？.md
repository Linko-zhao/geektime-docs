...
<li><span>罗杰</span> 👍（15） 💬（1）<p>impl Iterator for Equation&lt;Quadratic&gt; 判断返回 None 的地方是不是应该写成 `if self.current &gt;= u16::MAX as u32`，不然会有逻辑错误。</p>2021-10-20</li><br/><li><span>Marvichov</span> 👍（8） 💬（1）<p>和老师的对应下...

1. 使用泛型参数延迟数据结构的绑定；
2. 使用泛型参数和 PhantomData，声明数据结构中不直接使用，但在实现过程中需要用到的类型
3. 使用泛型参数让同一个数据结构对同一个 trait 可以拥有不同的实现。

4. 面向interface编程; 只不过静态多态
5. 引入自由参数 -&gt; 大部分impl共享; 剩下的, 根据自由参数类型的不同做template specialization -&gt; 比如tag struct -&gt; 本质还是代码共享
6. NewTypePattern; 一套代码给多个不同的type共用; 这个blog里面的例子比较生动: https:&#47;&#47;www.greyblake.com&#47;blog&#47;2021-10-11-phantom-types-in-rust&#47;; golang里面也经常用kilometer, mile来做例子, 类似于`type mile i32`;</p>2021-10-18</li><br/><li><span>Marvichov</span> 👍（4） 💬（2）<p>Cpp里面用tag和多generic param的例子也很多…
...
</p>2021-10-18</li><br/><li><span>Quincy</span> 👍（0） 💬（1）<p>1. 参数 F 是一个闭包，接收 Self::Item,返回 Fut 类型；
...
</p>2021-10-28</li><br/><li><span>渡鸦10086</span> 👍（2） 💬（0）<p>关于思考题：
f 是一个闭包，以 `Self::Item` 类型作为输入，以一个实现了 `Future` trait 的类型 Fut 作为输出，其中`Future` 的关联类型 `Output` 是 unit 类型</p>2022-01-16</li><br/><li><span>Geek_7c0961</span> 👍（1） 💬（0）<p>老师现在implement trait 用在返回值的类型了,那么 trait obejct还有什么用处么?它的性能那么差.
https:&#47;&#47;doc.rust-lang.org&#47;rust-by-example&#47;trait&#47;impl_trait.html</p>2022-05-16</li><br/><li><span>A.Y.</span> 👍（1） 💬（0）<p>关于第三点使用场景，似乎可以用于替代其他面向对象语言中有而rust不支持的类继承。</p>2022-05-07</li><br/><li><span>🐲礁sir</span> 👍（0） 💬（1）<p>终于明白substrate的frame&#47;executiv&#47;src&#47;lib.rs里面这段代码的用意：</p>2022-09-11</li><br/><li><span>zahi</span> 👍（0） 💬（0）<p>phantom type 的例子，感觉像java中的继承。</p>2022-08-14</li><br/>
</ul>

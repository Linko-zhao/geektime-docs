...
<li><span>Cryhard</span> 👍（11） 💬（1）<p>ES6等后续版本如果解决了浏览器兼容性问题，后续不再需要“编译”回老版脚本语言，从而获得了性能保障。这样的情况是否会成为wasm的重大阻碍呢？</p>2020-09-08</li><br/><li><span>Jupiter</span> 👍（9） 💬（1）<p>老师好，Typescript 可以设置参数类型，现在的项目中也经常使用，但是最后Typescript的代码也会被解释成Javascript, 所以Typescripts是不是只是规范程序员写代码，对于应用的性能其实没有什么帮助？</p>2020-09-08</li><br/><li><span>Vfeelit</span> 👍（4） 💬（1）<p>JS虽然动态，在TS把持下，jit 不进行或几乎不进行反优化，性能已经相当不错了吧。wasm不也通过vm转一圈最终产生机器码?</p>2021-01-13</li><br/><li><span>Vfeelit</span> 👍（1） 💬（1）<p>实现wasm的是虚拟机，任何语言，编译为某个中间语言并不是很困难的事。类似基于jvm, 它之上除了java还有其他语言，虽然都是基于jvm，但生态各不同。wasm 相比下不过就是一套vm规范，没有那么大想象力啊</p>2021-01-13</li><br/><li><span>地球外地人</span> 👍（1） 💬（1）<p>老师好，感觉ts代码虽然被编译成了js，但是转换后的js代码内部也避免了很多类型转换的问题，js引擎对于这一部分没有什么正反馈吗</p>2020-09-17</li><br/><li><span>donglin</span> 👍（0） 💬（1）<p>文中说ASM.js会被编译成机器码保存，下次再次执行时便会直接使用已保存的机器码，从而提高性能。

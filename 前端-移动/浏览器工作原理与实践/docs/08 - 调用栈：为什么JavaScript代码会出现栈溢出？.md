...
<li><span>黄晓杰</span> 👍（37） 💬（2）<p>老师，我有一个疑问，调用栈是后进先出，那么当存在闭包时，某个函数的执行上下文还存在，那么其他函数的出栈是否受影响？</p>2019-09-11</li><br/><li><span>mfist</span> 👍（20） 💬（12）<p>1. 改成尾递归调用（需要在严格模式下面生效）
function runStack (n, result=100) {
  if (n === 0) return result;
  return runStack( n- 2, result);
}
runStack(50000, 100)
2. 改成循环调用，不使用递归函数，就不存在堆栈溢出</p>2019-08-22</li><br/><li><span>奕</span> 👍（10） 💬（5）<p>关于调用栈的大小，不用的平台，比如浏览器，nodejs 怎么查看设置的，还是硬编码的？</p>2019-08-24</li><br/><li><span>心飞扬</span> 👍（8） 💬（3）<p>addAll函数中的result并不在变量环境中，而是执行完add后才被放在this中</p>2019-11-14</li><br/><li><span>pyhhou</span> 👍（8） 💬（1）<p>思考题当中的函数，如果输入参数是正偶数，那么不管数值多大，最后结果都是 100，除此之外，如果输入参数是负数或者是正奇数，甚至说是浮点数，那么使用递归方式调用会导致栈溢出，使用循环方式去实现会导致死循环，如果仅仅是基于当前的输入参数（50000）改写的话：
function runStack (n) {
  return 100;
}
runStack(50000);
...
runStack(50000);</p>2019-08-23</li><br/><li><span>Claire</span> 👍（7） 💬（1）<p>运用尾递归，其实尾递归也会产生栈溢出问题，但是查资料看，很多编译器已经优化了尾递归，当编译器检测到一个函数调用是尾递归的时候，它就覆盖当前的活动记录而不是在栈中去创建一个新的</p>2019-08-23</li><br/><li><span>许童童</span> 👍（7） 💬（5）<p>将递归改成迭代就好了，还可以使用尾递归优化。感觉老师这道题改成斐波那契数列会更好。
...
runStack(50000)</p>2019-08-22</li><br/><li><span>Geek_East</span> 👍（4） 💬（1）<p>执行上下问的本质是一个object吗</p>2019-12-06</li><br/><li><span>tick</span> 👍（3） 💬（2）<p>老师，我可不可以这样理解，您所说的调用栈并不是严格意义上的栈，因为在addAll中调用add时，add的函数代码还是在全局上下文中，即此时栈中有全局上下文，addAll上下文，但此时还需要去访问栈底的全局上下文中取出add的函数代码，这样是不是不算是严格意义上的栈？</p>2019-08-25</li><br/><li><span>爱吃锅巴的沐泡</span> 👍（3） 💬（1）<p>老师，有两个问题：
1、老师在文中写到“首先，从全局执行上下文中，取出 add 函数代码。”，这里是取到函数的引用，还是整个函数代码，函数的存储是怎样的？
2、声明带参的函数并调用的编译过程是怎样的，参数应该是和arguments有关吧，老师能详细说一下编译过程嘛？</p>2019-08-22</li><br/><li><span>林展翔</span> 👍（1） 💬（2）<p>老师, 可以问一下, 除了去捕获异常以外, 有没有什么办法能够跳过异常语句去执行剩下的 JS 语句.</p>2019-08-22</li><br/><li><span>Jim</span> 👍（1） 💬（1）<p>老师，您的执行上下文图里都会有一个变量环境和词法环境，可是为什么词法环境没有东西呢？请问变量环境和词法环境的区别是什么呢？</p>2019-08-22</li><br/><li><span>江霖</span> 👍（0） 💬（1）<p>
var a = 2
function add(b,c){
return b+c
}
...
addAll(3,6)
老师这段代码的result是不是应该在全局的context下</p>2019-11-14</li><br/><li><span>无名</span> 👍（0） 💬（2）<p>看文中的代码表达式中大部分结尾都没有加“;”号；JS规范中建议加还是不加“;”号？</p>2019-08-29</li><br/><li><span>阿段</span> 👍（0） 💬（1）<p>算法经典思想：循环消除尾递归</p>2019-08-26</li><br/>
</ul>

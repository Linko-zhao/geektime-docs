...
app.get("/track.gif", (req, res) => {
  ...
  console.log("Received tracking data: ", data); // 返回一个1x1像素的透明GIF图片
...
app.get("/track.gif", (req, res) => {
  ...
  logger.info(data); // 返回一个1x1像素的透明GIF图片
...
export class BaseTrace implements BaseTraceInterface {
  // 存储链路日志数据
  public queue: TraceData[] = []; // 发送请求时间间隔
...
export class BaseTrace implements BaseTraceInterface {
  ...
  public static init(options: TraceOptions): BaseTrace {
    const traceSdk = new BaseTrace(options); // ...省略部分代码
    setInterval(() => {

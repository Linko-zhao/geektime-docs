...
<li><span>Hello,Tomrrow</span> 👍（32） 💬（2）<p>JS 中的const 是否是可变，要分情况。
如果 const 声明的变量赋值给了原始类型，如数字、bool、字符串，此时就是不可变的；
如果 const 声明的变量赋值了复合类型，如数组、对象，此时变量指向的地址不可比变，但是复合类型的内容还是可以调整的， 如更改对象的属性值</p>2022-09-21</li><br/><li><span>null</span> 👍（11） 💬（1）<p>单独这一篇就值回票价了，学到了很多东西😁</p>2022-09-19</li><br/><li><span>Geek_dc85eb</span> 👍（5） 💬（1）<p>开发中const一般是不变的</p>2022-09-20</li><br/><li><span>I keep my ideals💤</span> 👍（4） 💬（1）<p>我认为const对于值类型来说是不可变的，但是对应引用类型来说就不一定了(可以保证引用类型的存储地址不变，但是不能保证里面存储的内容不变)</p>2022-09-20</li><br/><li><span>海是蓝天的倒影</span> 👍（3） 💬（1）<p>这是我见过函数式和面向对象讲得最透彻的，没有之一。
感谢老师</p>2022-12-13</li><br/><li><span>依韵</span> 👍（2） 💬（1）<p>&gt; “纯函数”对值只影响一次，而“不可变”完全不影响。
...
如 `conat add = (a,b) =&gt; a + b;` add 是纯函数，也是不可变。</p>2022-10-13</li><br/><li><span>weineel</span> 👍（2） 💬（3）<p>副作用的概念有点刷新认知。
老师说的副作用，指函数不可控的外部环境。
我之前认为的副作用，是函数会对外部环境产生影响，比如改变全局变量的值。

还有一个疑问，如果函数只改变了全局变量的值，并没有使用它进行计算(相同的输入，会有相同的输出)，这个函数还是纯函数吗？

按我之前的对副作用的理解，就会认为不是纯函数了。</p>2022-10-09</li><br/><li><span>南城</span> 👍（2） 💬（1）<p>试读白嫖！能免费看四章（催更帖）</p>2022-09-20</li><br/><li><span>雨中送陈萍萍</span> 👍（1） 💬（1）<p>老师讲的确实不错！！

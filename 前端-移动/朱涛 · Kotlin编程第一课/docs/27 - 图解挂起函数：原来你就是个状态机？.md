...
​ 如果一个挂起函数挂起，它的返回值会是 CoroutineSingletons.COROUTINE_SUSPENDED；</p>2022-04-12</li><br/><li><span>Geek_Adr</span> 👍（4） 💬（1）<p> &#47;&#47; JAVA 调用 挂起函数
...
</p>2022-04-10</li><br/><li><span>Shanks-王冲</span> 👍（4） 💬（1）<p>思考题：后知后觉
...
*关于如何在Java中调用：之前有课程提到，在Java中的getContext()中可考虑传入EmptyCoroutineContext.INSTANCE</p>2022-03-30</li><br/><li><span>Allen</span> 👍（3） 💬（1）<p>关于思考题的思考：

在 Java 中是可以调用挂起函数的，当挂起函数通过 Kotlin 编译器进行 CPS 转换后，对应的挂起函数就被转换成了带 Callback 参数（Continuation 接口）的普通函数，只要传入 Continuation 接口的实现就可以了。</p>2022-03-21</li><br/><li><span>白</span> 👍（1） 💬（2）<p>我不是很明白是怎么恢复协程，也就是调用resumewith，java反编译过去也没看到调用</p>2022-05-04</li><br/><li><span>神秘嘉Bin</span> 👍（1） 💬（1）<p>HelloSuspendTestKt.hello(new Continuation&lt;Integer&gt;() {
@NotNull
@Override
public CoroutineContext getContext() {
return EmptyCoroutineContext.INSTANCE;
        ...
        });</p>2022-03-24</li><br/><li><span>H.ZWei</span> 👍（0） 💬（1）<p>有点类似rxjava操作符的嵌套</p>2022-04-28</li><br/><li><span>荷兰小猪8813</span> 👍（0） 💬（1）<p>1、invokeSuspend 不是协程的入口，是 resumeWith 的入口，

恢复协程是调用的 continuation.resumeWith，这个会调用 invokeSuspend</p>2022-04-11</li><br/><li><span>荷兰小猪8813</span> 👍（0） 💬（2）<p>这样看，每个挂起函数，都会创建一个继承了 ContinuationImpl 的匿名内部类对象，把传进来的 Continuation 包起来～～～</p>2022-04-11</li><br/><li><span>荷兰小猪8813</span> 👍（0） 💬（1）<p>withContext 为啥没有创建新的协程呢？？？我看源码都有对应的 job 和 coroutome.start</p>2022-04-11</li><br/><li><span>杨小妞</span> 👍（0） 💬（1）<p>以下代码： 挂起函数b中的c、d之间不存在回调关系，而挂起函数a的b，a、b之间存在回调关系？可以这么理解吗？
suspend fun a() {
b()
}
